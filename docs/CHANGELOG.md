...
- Muyu: every strike is also POSTed to the user's personal inbox (`api.gudong.site/inbox/{token}`) as a `Merit +N` note. Wi-Fi STA connect (`main/muyu_wifi.c`) and the HTTPS POST client (`main/muyu_inbox.c`) run on dedicated FreeRTOS tasks; strikes are batched ~every 2 s and queued while offline so the daily-limit cannot be exhausted by fast tapping. Token and Wi-Fi credentials are supplied at build time as `main/muyu_creds_local.h`, which `tools/validate.sh` generates from the `MUYU_*` environment variables and deletes after the build (see `main/muyu_creds.h`); they never live in the public repo or in CI logs. New host test `tests/test_muyu_inbox.c` covers the JSON body builder (single strike, batch, large numbers, buffer-too-small). `build-firmware.yml` reads `MUYU_INBOX_TOKEN` / `MUYU_WIFI_SSID` / `MUYU_WIFI_PASS` from repository secrets; local builds pass them as environment variables to `tools/validate.sh --firmware`.
